...
local lua_server_available, lua_requested_server = lsp_installer_servers.get_server("sumneko_lua")
if lua_server_available then
    lua_requested_server:on_ready(function ()
        lua_requested_server:setup {
          settings = {
            Lua = {
              diagnostics = {
                  globals = {'vim'},
    ...
    if not lua_requested_server:is_installed() then
        lua_requested_server:install()
    end
end
